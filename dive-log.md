...
- **⭐ Flat Rock / Palos Verdes (8/10) — MISS ✗ (under-call; FIRST real PV dive report).** Reported: "**viz around 20 ft**, temps 75 surface / 72 bottom, slight surface currents, **great conditions**." Tool `pv` **12–16 ft (green water)** — chl **7.64**, WE 20 (calm), 73.3°F. **Under-call ~4–8 ft.** The satellite chl (7.64) is painting PV green when the water was a clear 20. PV has **no turbidity sensor** (runs on wave-energy + chl), so a nearshore satellite chl over-read (common over rocky/kelpy headlands — same failure mode as the NorCal spots and the bloomSheltered spots) drags it into the mid-teens green. **First PV data point ever — can't act on one, but flag: if PV keeps beating its green-ish read, it likely wants a `bloomSheltered`-style chl cap (it's a sheltered rocky headland, exactly that profile).** Partially answers the earlier "what did the tool predict for PV" — it's been ~12–17 green, and today's report says PV is running clearer than that.
