...
- **FIX WRITTEN (2026-07-30, pending deploy):** added an **ljshores-only** relief in that block — ease the cut toward neutral (0.92) when NTU is live & clean (<0.35), swell is long-period (≥14s clean groundswell), AND the Shores' OWN rawWE ≤110. Rationale: ljshores is co-located with the Scripps NTU sensor + PIERViz cam, so its turbidity is a DIRECT reading, not a 1-mi proxy — don't bury it with the Cove's borrowed exposure. Guard-rails (verified by regression harness): 7/09 churned day (rawWE 153) untouched, 6/28 short-period surface churn untouched (needs ≥14s), Cove/MR keep the full cluster dampener (ljshores-only), bloom/murky days untouched (needs NTU<0.35). Effect: Shores **7–11 → 11–16**. **Not yet deployed** — worker repo isn't git-tracked and wrangler isn't logged in; Bret to run `cd ~/Desktop/spearfactor-api && npx wrangler login && npx wrangler deploy`.
