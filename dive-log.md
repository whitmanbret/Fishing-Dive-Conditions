...
- **THE REAL FIX (proposed, not yet shipped): give the Breakwater the `bloomSheltered` treatment.** Point Lobos & Hopkins already carry `bloomSheltered:true`, which caps the chl fed to the viz model at 10 ("green/bloom," not "pea soup") because a sheltered spot's surface clears better than the raw satellite bloom implies. The Breakwater/San Carlos is exactly that kind of sheltered shallow entry — it should read ~green-bloom (≈10–17), NOT pea-soup 3–5, during an open-bay bloom, with the `#vizStratNote` warning deep divers. This is the correct, pattern-consistent fix (not another hold tweak). Awaiting Bret's OK.
