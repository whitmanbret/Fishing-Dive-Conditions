...
- **⚠️ STILL OPEN — cam-vs-model gap (NOT yet fixed).** Even after Bug 1, the Shores MODEL reads **3–6 ft** while the **cam (direct ground truth at the pier) sees 11–14**. Driver: **ntu 0.7 + WE ~117 (swell surge)** push the sensor model low, but the cam looks through the actual water and sees clearer. The cam wire-in is **DAMPEN-ONLY** (can only lower viz), so it sits inert here and can't correct an under-call. This is the case FOR letting the cam gently **LIFT** the Scripps-proximity estimate when it confidently sees clearer water than the sensors — but that inverts the safety assumption (over-call direction), so **flagged to Bret, not shipped.** (Also a data point for the eventual cam→NTU recalibration: ntu 0.7 ≈ 11–14 ft cam viz, i.e. our NTU curve is too harsh at 0.7.)
