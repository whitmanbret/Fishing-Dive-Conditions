...
- **⭐ Goff / Treasure Island (8/10) — MISS ✗ (2nd clear-day under-call; FIX CANDIDATE).** Reported: "**Conditions were 25+ today at Goff.**" Tool `treasureisland` **11–16 ft** — chl **0.22 (gin-clean, no bloom)**, swell **2 ft @ 14 s** (tiny, long-period), WE 56, 69.9°F, no surf note. **Under-call ~9+ ft**, and it's genuine (tool BELOW diver, so diver-underestimate makes it worse, not better). **This is the 2nd clear-day Goff under-call** (8/7: 25+ vs tool 14–19). **Likely cause:** Goff runs on the wave-energy fallback (no turbidity sensor) with **no gin-clear lift** (that's ljshores/ljcove only), and today's **2 ft long-period swell inflates WE to 56** — which drags a genuinely gin-clear (chl 0.22), tiny-surf spot down to 11–16. The long-period swell pumps the WE number without actually muddying a sheltered-ish reef. → **FIX CANDIDATE (2 corroborations, well-supported): ease Goff's WE penalty when the swell is small-but-long-period AND the water is clean (low chl), so a calm gin-clear day isn't capped in the mid-teens.** Proposed to Bret, NOT shipped — want a careful design (and to not over-correct after the Monterey lesson). Contrast the surf-day behavior: 8/9 Goff read 7–12 (leaned HIGH) on WE 109 — so the WE response needs care in both directions.
