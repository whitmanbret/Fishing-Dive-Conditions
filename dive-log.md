...
- **La Jolla Cove (8/10, AM + reef) — mild OVER-call ✗ (gin-clear stealth-haze, 2nd corroboration).** Reports: 8–9am "vis **decent, could see the bottom the entire swim, just a little green and cloudy**" (and COLD — 73–75°F board but felt colder, upwelling); a reef diver "**good visibility around 20'**" (topes in 15' water, yellowtail, juvenile GSB, salema, baby blacksmith). So **~15–20, green/cloudy.** Tool `ljcove` **22–27 ft (green tinge)** — the `_coveGinClear` lift fired (ntu **0.18**, chl 0.48). **Mild over-call ~2–7 ft.** Same failure as 8/8: **clean pier NTU but a recent bloom leaves the water green/hazy** — chlRecentMax **6.46** flags the recent bloom the instantaneous NTU can't see. **Now 2 corroborations (8/8, 8/10) that the gin-clear lift over-fires when chlRecentMax is elevated.** → **Candidate fix: add a `chlRecentMax` guard to `_coveGinClear`** (don't take the full lift if a bloom was in the 24h window). **BUT verify first:** the 8/7 success (Cove 30 ft, lift correct) may ALSO have had elevated chlRecentMax — if so, a naive guard would regress it. Need to check 8/7's chlRecentMax before shipping. NOT changed yet.
